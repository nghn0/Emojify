...
# Build CNN model (same as your training model)
def build_model():
    model = Sequential([
...
# Load trained model weights (assuming weights file is available)
emotion_model = build_model()
emotion_model.load_weights('emotion_model.weights.h5')  # Ensure this is your correct file path
...
# Function to display video feed and detect emotion
def show_vid():
    ...
    # Detect the best face from the detected bounding boxes
    ...
    if best_box is not None:
        ...
        try:
            ...
            show_text[0] = int(np.argmax(prediction))  # Update the emotion label
        except:
            pass

    # Convert the frame to RGB and display it on the Tkinter window
...
# Function to update the emoji display based on detected emotion
def show_vid2():
...
# Start the threads to show video and update the emoji
